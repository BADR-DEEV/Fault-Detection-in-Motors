...
# ---------------- load dataset (your section_dataset logic) ----------------
def load_mat_file(path):
    ...
    if "H" in m:
        arr = m["H"].squeeze()
        if arr.ndim == 1:
            # try to reshape to (5000,3) if flat
            if arr.size == 5000*3:
                arr = arr.reshape(5000, 3)
        return arr
    # try other keys
    for k in m:
        if isinstance(m[k], np.ndarray) and m[k].ndim >= 1:
            arr = m[k].squeeze()
            if arr.ndim == 2 and arr.shape[1] in (1,3):
                return arr
    raise KeyError("No 'H' variable found in mat file: " + path)

def section_dataset():
    healthy = {"fileName": [], "x": [], "label": 0}
    faulty  = {"fileName": [], "x": [], "label": 1}
    # load healthy
    for fname in sorted(os.listdir(HEALTHY_DIR)):
        if not fname.lower().endswith(".mat"): continue
        path = os.path.join(HEALTHY_DIR, fname)
        arr = load_mat_file(path)
        healthy["fileName"].append(fname)
        healthy["x"].append(arr)
    # load faulty
    for fname in sorted(os.listdir(FAULTY_DIR)):
        if not fname.lower().endswith(".mat"): continue
        path = os.path.join(FAULTY_DIR, fname)
        arr = load_mat_file(path)
        faulty["fileName"].append(fname)
        faulty["x"].append(arr)
    return healthy, faulty

# ---------------- EMD helper: ensure up to MAX_IMFS, drop IMF1 ----------------
try:
    from emd_imfs import sig_to_imf as user_sig_to_imf
except Exception:
