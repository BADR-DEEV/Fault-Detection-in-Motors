...
def process_file_multifault(file_path, label_name, file_id_counter):
    ...
    try:
        ...
        return feats,labels, rpm_vals, file_ids
    except Exception as e:
...
def load_mafaulda_dataset_with_groups(base_path, cache_file="d.pkl"):
    """Loads dataset with file_id tracking AND severity metadata for physics validation"""
    cache_path = Path(cache_file)
    if cache_path.exists():
        logger.info(f"✅ Loading cached dataset: {cache_file}")
    ...
    for class_name, config in DATA_SOURCES.items():
        ...
        for file_info in files_found:
            ...
            f_feats, f_labels, f_rpms, f_ids = process_file_multifault(f_path, class_name, global_file_counter)
...
def plot_confusion_matrix_academic(y_true, y_pred, class_names, save_path=None):
    ...
    axes[1].set_title("Confusion Matrix (Row Normalized %)", fontsize=16, fontweight='bold')
    ...
    # ================================
    # CORRECT NORMAL FALSE ALARM RATE
    # ================================
    normal_idx = class_names.tolist().index("Normal")

    false_alarm_rate = 1 - cm_norm[normal_idx, normal_idx]

    print("\n==============================")
    print("PHYSICS VALIDATION")
    print("==============================")
    print(f"Normal false alarm rate: {false_alarm_rate:.2%}")

    if false_alarm_rate < 0.05:
        print("✅ Excellent Normal class isolation (<5%)")
    else:
        print("⚠️ High false alarm rate")
...
def plot_tsne_3d_only(X_scaled, y, class_names, perplexity=30):
    """
    Pure 3D t-SNE visualization (NO severity, NO extra physics metadata)
    """
    ...
    logger.info("🎨 Generating CLEAN 3D t-SNE (No Severity)...")
    ...
    fig = px.scatter_3d(
        ...
        height=750,
        title=f"3D t-SNE Fault Clusters (Perplexity={perplexity})"
    )

    # Uniform size (NO severity influence)
    ...
    logger.info("✅ Clean 3D t-SNE complete")
# ==================== MAIN PIPELINE ====================
...
if RUN_FEATURE_IMPORTANCE:
    ...
    if PLOT_PER_CLASS_ROC:
        severity_values_test = df['severity_value'].values[test_idx]
        severity_types_test = df['severity_type'].values[test_idx]
    ...
    joblib.dump(saved_pipeline, r"C:\DEV\personal\Ai-Driven-Vibrations-motor\src\training_cleaned\svm_pipeline_physics_validated.pkl")
